...
parquet can show different row counts than the 11,714 figure quoted above if they
were last generated by separate, unsynced pipeline runs. Re-run the full pipeline
(`python .\scripts\run_pipeline.py` followed by `python .\scripts\run_ml.py`) in
one sitting before quoting any of these numbers together (e.g. in an interview or
on a resume), so all three artifacts reflect the same data snapshot.
...
Important features include (ranked by actual SHAP/feature-importance magnitude in `reports/feature_importance.csv`, regenerated after the target-leakage fix below):

- CVSS base score
- Reference count
- Privileges required (none / low)
- CVSS severity (critical / medium)
- Attack vector (network)
- User interaction (none)
- CWE category (e.g. CWE-94 code injection, CWE-244)
